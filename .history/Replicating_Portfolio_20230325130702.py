...
class Insurance_Portfolio():
    ...
    def simualte_Ypaths_Bpaths(self):
        timer=perf_counter()
        """ Simulate Fund Price """
    ...
    def Replicating_Portfolio(self):
        ...
        for t_i in trange(self.n_time_steps_Y-2, -1, -1):
            ...
            model.fit(X1, values[:,t_i+1], epochs=epochs, validation_split=0.0, verbose=1, batch_size=512, callbacks=callabacks) #, initial_epoch= 200 if t_i != n_time_steps-2 else 0)
    ...
    @property
    def Replicaitng_Portfolio_V0(self, changed_param:list = None, return_info=False, make_plots=False):
        if changed_param:   temp_stored_original_value = self.parameters[changed_param[0]]

        self.simualte_Ypaths_Bpaths()
        self.Replicating_Portfolio()

        if make_plots:
            V0_RP = self.values[:,0].mean() * ADJUSTMENT_FACTOR

            qs = np.quantile(self.values, q=[.99, .95, .9, .1, .05, .01], axis=0) * ADJUSTMENT_FACTOR
            t  = np.linspace(0, T, n_time_steps_Y)
