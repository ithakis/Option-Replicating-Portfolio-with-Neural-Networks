...
class Insurance_Portfolio():
    ...
    def Replicaitng_Portfolio_V0(self, changed_param:tuple = None, return_info=False, make_plots=False):
        ...
        # Local Values
        Y_paths=self., B=B, n_time_steps_Y=n_time_steps_Y, out_of_money_P=out_of_money_P, S=S
        if make_plots:
        ...
        if return_info:
            ...
            print(f'Discounted Eq[S] ADJ          = {self.S.mean() * np.exp(-self.r*self.T) * self.ADJUSTMENT_FACTOR:,.3f}')
            print(f' Difference                   = {V0_RP - S.mean() * np.exp(-self.r*self.T) * self.ADJUSTMENT_FACTOR:,.3f} : {(V0_RP - self.S.mean() * np.exp(-r*T) * ADJUSTMENT_FACTOR)/V0_RP * 100:,.3f}% ')
