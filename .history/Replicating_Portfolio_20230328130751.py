...
class Insurance_Portfolio():
    ...
    def Replicaitng_Portfolio_V0(self, changed_param:tuple = None, print_info=False, make_plots=False):
    ...
    def Simulate_N_T_outcomes(self, params: dict):
        ...
        n_time_steps = int(params['T']/paramsdt)+1 ; print(f'Number of steps: {n_time_steps}')


        """ Simulate Fund Price """
        W2      = sobol_norm(n_paths_N, d=n_time_steps)
        L_paths = np.empty((2**n_paths_N, n_time_steps))
        L_paths[:,0] = l0
        for t in range(1,n_time_steps):
            L_paths[:,t] = L_paths[:,t-1] + (c * L_paths[:,t-1] * _dt + ita * np.sqrt(_dt)*W2[:,t]).squeeze()

        N_paths = np.empty((2**n_paths_N, n_time_steps), dtype=int)
        N_paths[:,0] = N
        """ Binomial Distribution of N(t) """
        for t in range(1,n_time_steps):
            probabilities = np.exp(-L_paths[:,t]*_dt)
