...
class Insurance_Portfolio():
    ...
    def Replicaitng_Portfolio_V0(self, changed_param:tuple = None, print_info=False, make_plots=False, return_phi_psi=False):
        ...
        if changed_param and not self.simulated:
            ...
            if return_phi_psi:
                local_values   = self.Replicating_Portfolio(Y_paths=Y_paths, B=B, n_time_steps_Y=n_time_steps_Y, out_of_money_P=out_of_money_P, S=S, local=True, return_phi_psi=True                
        else:
            # Local Values
            Y_paths=self.Y_paths; B=self.B; n_time_steps_Y=self.n_time_steps_Y; out_of_money_P=self.out_of_money_P; S=self.S
            local_values = self.values
